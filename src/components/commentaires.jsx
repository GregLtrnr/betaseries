...
export default function Commentaires(props) {
  ...
  useEffect(() => {
    axios
      .get(
        "https://api.betaseries.com/comments/comments?type=" +
          props.type +
          "&client_id=" +
  ...
  function sendCommentaire() {
    if (text !== previousComment) {
      ...
      if (localStorage.getItem("token")) {
        axios
          .post(
            "https://api.betaseries.com/comments/comment?type=" +
              props.type +
              "&client_id=" +
              process.env.REACT_APP_APIKEY +
              "&id=" +
      ...
      <div className="flex flex-col items-center border">
        ...
        {error && (
          <p className="text-red-500 text-center">
            Votre commentaire a déjà envoyé.
          </p>
        )}
        {errorNetwork && (
          <p className="text-red-500 text-center">{errorNetworkContent}</p>
        )}
        <button
          className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded"
          onClick={() => {
