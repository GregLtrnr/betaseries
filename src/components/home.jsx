...
export default function Home() {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        "https://api.betaseries.com/shows/discover?client_id=" +
    ...
    axios
      .get(
        "https://api.betaseries.com/movies/discover?client_id=" +
          process.env.REACT_APP_APIKEY +
          "&limit=30&type=popular"
      )
      .then(function (response) {
    ...
    axios
      .get(
        "https://api.betaseries.com/shows/random?client_id=" +
          process.env.REACT_APP_APIKEY +
          "&nb=30"
      )
      .then(function (response) {
        setRandomSeries(
          response.data.shows.filter((serie) => serie.images.poster !== null)
        );
      })
      .catch(function (error) {
        console.log(error);
              ...
              >
                <img
                  ...
                  src={
                    serie.images.poster
                      ? serie.images.poster
                      : "https://via.placeholder.com/246x362"
                  }
                  alt=""
                />
                <FollowButton
                  id={serie.id}
                  followed={
                    userSeries.find((userSerie) => userSerie.id === serie.id) !=
                    undefined
                      ? true
                      : false
                  }
                ...
                <a
                  href={"http://localhost:3000/series/" + serie.id}
          ...
          popularFilms.map((film, index) => {
            return (
              ...
              >
                ...
                <a
                  href={"http://localhost:3000/film/" + film.id}
                  className="btn btn-blue bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 mb-2 rounded justify-center"
                >
                  Voir le film
                </a>
