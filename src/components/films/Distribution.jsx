...
export default function DistributionMovie(props){
    ...
    useEffect(() => {
        axios.get('https://api.betaseries.com/movies/characters?client_id=' + process.env.REACT_APP_APIKEY + '&id=' + props.id)
                ...
                {acteurs && acteurs.map((char,index)=>{
                    return(
                        ...
                        <div  key={index} onClick={()=>{actorPage(char.person_id)}} className="flex flex-col items-center mx-4" style={{minWidth:'200px', width: '200px', height : '380px'}}>
                            <img src={`https://api.betaseries.com/pictures/persons?client_id=${process.env.REACT_APP_APIKEY}&id=${char.person_id}` ? `https://api.betaseries.com/pictures/persons?client_id=${process.env.REACT_APP_APIKEY}&id=${char.person_id}` : 'https://via.placeholder.com/200x300'} alt="" />
