...
export default function FilmShow() {
  ...
  useEffect(() => {
    axios(
      `https://api.betaseries.com/movies/movie?client_id=${process.env.REACT_APP_APIKEY}&id=${id}`
    )
        ...
        <div className="flex flex-col items-center md:items-stretch lg:flex-row">
          ...
          <div className="flex flex-col mr-4">
            <h1 className="text-4xl font-bold mb-4">
              {film && film.title} ({film && film.production_year})
            </h1>
            ...
            <div className="flex">
              {film &&
                film.platform_links.map((stream, index) => {
                  return (
                    <a href={stream.link}>
                      <p
                        key={index}
