...
export default function SaisonsEpisodes(props) {
  ...
  useEffect(() => {
    axios
      .get(
        "https://api.betaseries.com/shows/seasons?client_id=" +
      ...
      .then(function (response) {
        ...
        axios
          .get(
            "https://api.betaseries.com/shows/episodes?client_id=" +
              process.env.REACT_APP_APIKEY +
              "&id=" +
              props.id +
              "&access_token=" +
  ...
  return (
    <div className="w-full">
          ...
          saisons.map((saison, index) => {
            return (
              <div
                key={index}
                onClick={() => {
                  setCurrentSaison(saison.number);
                }}
                className="flex flex-col items-center mx-4"
                style={{ minWidth: "150px", width: "150px", height: "250px" }}
              >
                <img
                  src={
                    saison.image
      ...
      <div className="flex  items-center hover:overflow-x-scroll overflow-hidden hover:overflow-y-hidden hover:cursor-pointer">
        {episodes &&
          episodes
            .filter((episode) => episode.season === currentSaison)
            .map((episode, index) => {
              ...
              return (
                <div
                  ...
                  style={{ minWidth: "250px", width: "250px", height: "220px" }}
                >
                  <ActionsOnEpisodes
                    id={episode.id}
                    list={episodes}
                  ...
                  <img
                    ...
                    src={iswatched ? Check : null}
                    alt=""
                  />
                  <a href={window.location.href + "/episode/" + episode.id}>
                    <img
                      src={
                        "https://api.betaseries.com/pictures/episodes?client_id=" +
                        process.env.REACT_APP_APIKEY +
                        "&id=" +
                        episode.id +
