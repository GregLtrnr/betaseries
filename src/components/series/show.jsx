...
export default function Show() {
    ...
    if (localStorage.getItem("token")) {
      axios
        ...
        .then(function (response) {
          ...
          if (
            response.data.shows.find((serie) => serie.id === slug) !== undefined
          ) {
            setAdded(true);
          }
          if (
            response.data.shows.find((serie) => serie.id === slug).user.archived === true
          ) {
            setArchived(true);
          }
        })
  ...
  function followSerie() {
    axios
      .post(
        "https://api.betaseries.com/shows/show?client_id=" +
          process.env.REACT_APP_APIKEY +
          "&id=" +
          slug +
          "&access_token=" +
  ...
  function archiveSerie() {
    axios
      .post(
        "https://api.betaseries.com/shows/archive?client_id=" +
          process.env.REACT_APP_APIKEY +
          "&id=" +
          slug +
          "&access_token=" +
      ...
      <div className="mx-10 p-4 border-2 bg-white">
        <div className="flex flex-col items-center md:items-stretch lg:flex-row">
          ...
          <div className="w-full flex flex-col">
            <h1 className="text-5xl font-normal leading-normal mt-0 mb-2 text-gray-800">
              {serie && serie.title}
            </h1>
          ...
          <div className="w-1/2 flex flex-col">
            ...
            <div className="flex">
              {serie &&
                Object.keys(serie.genres).map((genre, index) => {
                  return (
            ...
            {!added && (
              <button
                className="w-full btn btn-blue bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded"
                onClick={() => {
                  setAdded(true);
                  followSerie();
                }}
            ...
            {added && (
              <button
                className="w-full btn btn-red bg-red-700 hover:bg-red-900 text-white font-bold py-2 px-4 rounded"
                onClick={() => {
                  unfollowSerie();
                  setAdded(false);
                }}
            ...
            {!archived && added && (
              <button
                className="w-full btn btn-orange bg-orange-700 hover:bg-orange-900 text-white font-bold mt-4 py-2 px-4 rounded"
                onClick={() => {
                  archiveSerie();
                  setArchived(true);
                }}
            ...
            {archived && added && (
              <button
                className="w-full btn btn-green bg-green-700 hover:bg-green-900 text-white font-bold mt-4 py-2 px-4 rounded"
                onClick={() => {
                  unarchiveSerie();
                  setArchived(false);
                }}
